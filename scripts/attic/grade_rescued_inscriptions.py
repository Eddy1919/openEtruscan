...
def grade_inscriptions():
    ...
    cur.execute(
        "SELECT cie_id, transliterated, latin_commentary FROM cie_review WHERE confidence IS NULL OR confidence = ''"
    )
    rows = cur.fetchall()
    ...
    for row in rows:
        ...
        try:
            print(
                f"[{graded_this_run + 1}/{total_remaining}] Grading CIE {cie_id}...",
                end="",
                flush=True,
            )
            prompt = (
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            "Gemini has performed a conservative philological assessment of the 1,280 rescued records.\n\n"
        )
