...
def flag_duplicates():
    ...
    cur.execute(
        "SELECT cie_id, transliterated, latin_findspot, latin_commentary, pdf_source FROM cie_review"
    )
    rows = cur.fetchall()
    ...
    for r in rows:
        ...
        if key not in clusters:
            clusters[key] = []
        clusters[key].append(
            {"id": cid, "text": trans, "findspot": find, "comment": comm, "pdf": pdf}
        )
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            f"Found **{len(duplicate_clusters)}** clusters of potential duplicates using fuzzy text normalization (s == ś).\n\n"
        )
        ...
        for i, (key, cluster) in enumerate(duplicate_clusters.items(), 1):
            ...
            for item in cluster:
                ...
                comm = (
                    (item["comment"][:50] + "...")
                    if item["comment"] and len(item["comment"]) > 50
                    else item["comment"]
                )
