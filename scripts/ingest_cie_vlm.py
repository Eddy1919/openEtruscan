            ...
            "items": {
                ...
                "properties": {
                    ...
                    "etruscan_text_transliterated": {
                        "type": "STRING",
                        "description": (
                            "Transliterated Etruscan text in Latin characters"
                        ),
                    },
...
def extract_page_images(pdf_path: Path, start_page: int, num_pages: int):
    ...
    for i in range(start_page, end_page):
        page = pdf[i]
        bitmap = page.render(scale=150 / 72)  # 150 DPI (smaller payload)
...
def process_with_gemini_rest(pil_image, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(
                f"    [Retry {attempt + 1}] Network Exception: {e}"
            )
            time.sleep(5)
...
def ingest_into_db(entries):
    ...
    for entry in entries:
        ...
        cursor.execute(
            "SELECT id FROM inscriptions WHERE id=?", (formatted_id,)
        )
...
def main():
    ...
    for page_num, img in images:
        print(
            f"Processing page {page_num} with Gemini 2.5 Flash REST API..."
        )
        ...
        if extraction and "entries" in extraction:
            ...
            print(
                f"  ⚠️ No entries found or error on page {page_num}"
            )
