...
def main():
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(
            "SELECT id, canonical, canonical_clean, raw_text, findspot, classification, medium, object_type, notes FROM inscriptions ORDER BY id"
        )
        rows = cur.fetchall()
    ...
    with conn.cursor() as cur:
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_emb_text_hnsw ON inscriptions USING hnsw (emb_text vector_cosine_ops)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_emb_context_hnsw ON inscriptions USING hnsw (emb_context vector_cosine_ops)"
        )
        cur.execute(
            "CREATE INDEX IF NOT EXISTS idx_emb_combined_hnsw ON inscriptions USING hnsw (emb_combined vector_cosine_ops)"
        )
    conn.commit()
