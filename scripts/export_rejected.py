...
def main():
    print(f"Connecting to Postgres to fetch rejected records...")
    conn = psycopg2.connect(DB_URL)
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("SELECT id, canonical, classification, provenance_status FROM inscriptions WHERE provenance_status = 'rejected' ORDER BY id;")
        rows = cur.fetchall()
