...
def generate_rescue_report():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "Scanned **1,009** records in `cie_latin.db` using a high-precision phoneme sieve.\n"
        )
        f.write(
            f"Identified **{len(candidates)}** records containing definitive Etruscan markers.\n\n"
        )
        ...
        for c in candidates:
            ...
            f.write(
                f"| {c['id']} | `{c['markers']}` | {text_snip} | {c['findspot']} | {c['commentary']} |\n"
            )
