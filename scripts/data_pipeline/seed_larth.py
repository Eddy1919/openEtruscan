...
def seed_corpus(csv_data: str, db_path: str = "data/corpus.db") -> int:
    """Import Larth CSV into the OpenEtruscan corpus (bulk mode)."""
    ...
    print(f"📥 Parsing {total_rows:,} rows...")

    buffer_dict: dict[str, Inscription] = {}
    skipped = 0
    ...
    for idx, row in enumerate(all_rows):
        ...
        inscription_id = row.get("ID", f"LARTH_{idx:05d}").strip()
        ...
        try:
            ...
            print(f"   ⚠️  Failed to normalize '{clean_text[:40]}': {e}")
            skipped += 1
            continue
        ...
        buffer_dict[inscription_id] = Inscription(
            id=inscription_id,
            raw_text=raw_text,
        ...
        if (idx + 1) % 1000 == 0:
            ...
            print(f"   📝 Parsed {idx + 1:,}/{total_rows:,} ({elapsed:.1f}s)")
    ...
    insert_time = _time.perf_counter() - t1

    print(f"✅ Inserted {count:,} inscriptions in {insert_time:.1f}s")
    print(f"   Total time: {_time.perf_counter() - t0:.1f}s")
...
def main():
    ...
    parser.add_argument("--db-url", default=None, help="Database connection URL (defaults to env)")
    ...
    if args.codex_only:
        ...
        seed_codex(db_path=args.db_url)
    else:
        if args.csv_path:
        ...
        seed_corpus(csv_data, db_path=args.db_url)

        if args.codex:
            print("\n📚 Importing Etruscan codex texts...")
            seed_codex(db_path=args.db_url)

    # Show a sample query from the actual DB
    print("\n🔍 Verifying Cloud SQL deployment...")
    corpus = Corpus.load(args.db_url)
    results = corpus.search(limit=3)
    print(f"   Total records in DB: {results.total}")
    for insc in results:
        print(f"   {insc.id}: {insc.canonical} ({insc.findspot})")
    corpus.close()
