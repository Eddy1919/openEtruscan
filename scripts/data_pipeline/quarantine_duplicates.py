...
def execute_quarantine():
    ...
    for cid, trans, _find, can in rows:
    ...
    for cid in to_discard_internal:
        cur.execute(
            "INSERT INTO cie_discarded SELECT *, 'Internal Duplicate' FROM cie_review WHERE cie_id = ?",
            (cid,),
        )
    ...
    for cid in to_discard_remote:
        ...
        if cur.fetchone():
            cur.execute(
                "INSERT INTO cie_discarded SELECT *, 'Remote Overlap (Larth Dataset)' FROM cie_review WHERE cie_id = ?",
                (cid,),
            )
            cur.execute("DELETE FROM cie_review WHERE cie_id = ?", (cid,))
