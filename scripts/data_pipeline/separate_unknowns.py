...
def migrate_unknowns(source_conn, target_conn, unknown_findspots):
    ...
    # Delete from source
    source_cur.execute(
        f"DELETE FROM cie_review WHERE trim(latin_findspot) IN ({placeholders})", unknown_findspots
    )
    source_conn.commit()
...
def main():
    ...
    dbs_to_process = ["cie_etruscan.db", "cie_latin.db", "cie_ranges.db", "cie_range_latin.db"]
