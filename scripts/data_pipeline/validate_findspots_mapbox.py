...
def main():
    ...
    cur.execute(
        "SELECT DISTINCT cluster_name FROM unique_findspots WHERE cluster_name IS NOT NULL AND status='CLUSTER_GENERATED'"
    )
    ...
    cur.execute(
        "SELECT id, cluster_name, gemini_lat, gemini_lon FROM unique_findspots WHERE status='CLUSTER_GENERATED'"
    )
    rows = cur.fetchall()
