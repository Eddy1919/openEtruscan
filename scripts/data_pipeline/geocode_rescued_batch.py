...
gemini_schema = {
    "type": "OBJECT",
    "properties": {
        "modern_location": {"type": "STRING", "description": "Modern name of the findspot."},
        "lat": {"type": "NUMBER", "description": "Latitude of the findspot."},
        "lon": {"type": "NUMBER", "description": "Longitude of the findspot."},
        "uncertainty_m": {
            "type": "NUMBER",
...
def geocode_batch():
    ...
    for row in rows:
        ...
        try:
            print(
                f"[{processed_count + 1}/{len(rows)}] Hardening CIE {cie_id}...", end="", flush=True
            )
            ...
            if result:
                ...
                if (transliterated == "N/A" or not transliterated) and result.get(
                    "salvaged_transliteration"
                ):
