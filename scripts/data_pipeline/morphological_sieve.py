...
def main():
    ...
    for row in all_rows:
        ...
        if match:
            print(
                f"[{cie_id}] Flagged as LATIN by morphological sieve. Trigger: '{match.group(0)}' in '{clean_text}'"
            )
    ...
    if rows_to_delete_ids:
        cur_et.execute(
            f"DELETE FROM cie_review WHERE rowid IN ({','.join('?' * len(rows_to_delete_ids))})",
            rows_to_delete_ids,
        )
        conn_et.commit()
