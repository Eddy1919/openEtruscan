...
- **Theonym and place-name alignment.** Etruscan deity and place
  ...
  correspondences. For Latinised theonyms/place-names, orthographic
  neighbours are sometimes recovered (`menrva→minerva`,
  `hercle→hercules`, `fanu→fanum`; theonym field@10 ≈ 0.33, n=3);
  treat as suggestive, not reliable.
- **Within-language semantic-field exploration.** For an Etruscan
  ...
  pater]`).
- **Multilingual nearest-neighbour browsing.** The storage / API
  layer is designed to accept other ancient languages (Phoenician,
  Faliscan, Oscan) in future; no non-Etruscan corpus is currently
  ingested or served.
...
### Through a local research API

```bash
curl 'http://localhost:8000/neural/rosetta?word=fanu&from=ett&to=lat&embedder=xlmr-lora-v4'
```

> **Note.** The old hosted `api.openetruscan.com` endpoint is **retired**, and
...
1. **Small held-out test split (n=22 pairs).** Confidence intervals
   are correspondingly wide. RG.4 in the evaluation roadmap adds
