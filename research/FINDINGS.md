...
2. **Theonym + place-name alignment.** Etruscan deity and place names
   ...
   correspondences. The system recovers a number of these (in top-k,
   not always at rank 1): `menrva→minerva` (rank 7),
   `hercle→hercules` (rank 5), `fanu→fanum` (rank 1). Treat as
   suggestive — overall semantic-field@10 is only ~0.12–0.19.
...
Rerank model: `BAAI/bge-reranker-v2-m3`, a leading multilingual
...
on Gemini 2.5 Pro via Vertex AI (billed to a now-retired GCP project),
...
```bash
# NOTE: api.openetruscan.com is RETIRED and the current public API
# (www.openetruscan.com/api) does not expose neural/rosetta. Run this against a
# local instance of the research API (src/openetruscan/api) on :8000.
