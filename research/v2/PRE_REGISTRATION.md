...
**Currently shipped:** **v2.0.4** (2026-08-08). Stream A (classification) was re-run on the text-disjoint split with a fresh 3-rater jury (Krippendorff α = 0.8557, candidate-gold n=167; Deviation §D below); the v2.0.2 Stream A numbers (α = 0.7649, n=143) are superseded and their gold labels are lost. Stream C (lacunae) v2.0.2 "Finding C" was **retracted as a harness artifact** and re-run at v2.0.3 (Deviation §B).
...
- **Clean re-run delivered (v2.0.4, 2026-08-08, same day)**: Stream A was re-run end-to-end on the text-disjoint 427/285 split. Full evidence, committed and SHA256-pinned: [`results/classify/`](results/classify/).
  - **Jury**: Claude Opus 4.8 + Gemini 3.1 Pro + Gemini 3.5 Flash, all on Vertex AI, 427×3 = 1,281 ratings, zero API errors. Krippendorff α = **0.8557** (v2.0.2: 0.7649), with the §B lineage caveat verbatim: two of three raters share the Gemini lineage, so α is inflated relative to a three-lineage panel. Llama-family and other MaaS raters were unavailable on the billing project (verified 2026-08-08); this mirrors the v2.0.3 lacuna panel, and a lineage-independent panel remains future work. The jury deviates from the §A roster for the same externally-forced reason §B documents.
  - **Candidate-gold**: n = **167** (unanimous, conf ≥ medium), queue 59, all-unsure 201. Class support: funerary 77 / ownership 55 / dedicatory 25 / boundary 6 / legal 4 / **votive 0 / commercial 0**; `macro_f1` averages over all 7 codebook classes, so its ceiling on this gold set is 5/7 ≈ 0.714 (same convention as every prior v2 number).
  - **Results** (train = 285 clean silver rows; 10,000-resample bootstrap, seed=42): CharCNN **0.399** [0.353, 0.435]; TF-IDF+NB **0.293** [0.255, 0.329]; MicroTransformer 0.252 [0.140, 0.338]; EmbeddingMLP 0.210 [0.181, 0.242]. The v2.0.2 figures are the historical record these replace, **not** a controlled comparison: the v2.0.2 gold labels are lost, the jury differs, and the train pool differed by the 27 leaked-twin rows.
  - **Finding A does NOT replicate**: paired bootstrap (same 167 rows, seed=42) puts CharCNN above TF-IDF+NB (Δ +0.106, p = 0.0025) and above MicroTransformer (Δ +0.147, p = 0.0023). The v2.0.2 claim that architecture does not matter among local-feature models is withdrawn for v2.0.4; character-level convolution separates from both alternatives. **Finding B replicates**: TF-IDF+NB beats EmbeddingMLP (Δ +0.083, p < 0.0001), though the gap narrowed.
  - **Harness bug disclosed**: `train_neural.py` destructured eval tuples as `(gold, _, id)` instead of `(_, gold, id)` and wrote the inscription *text* into the predictions files' `gold_label` field. In-memory metrics were computed from a separate, correct list and are unaffected; the committed prediction files were regenerated after the fix. Caught the same day by a paired-bootstrap sanity check returning degenerate deltas; the invariant-test suite did not cover this writer, consistent with this file's history of harness bugs surfacing at the evidence boundary.
  - **Status of the headline**: `0.313` retires as "v2.0.2, contaminated split, gold set lost". The citable Stream A number is now **TF-IDF+NB macro F1 0.293 [0.255, 0.329] on v2.0.4 candidate-gold (n=167)**, still LLM-consensus silver, not philologist-ratified; the v2.0.4 handoff bundle ([`handoff/v2.0.4-etr/`](handoff/v2.0.4-etr/)) is regenerable from committed evidence and awaits two adjudicators.
