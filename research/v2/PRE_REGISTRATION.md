...
- **Why the frozen split was NOT regenerated**: the committed 400-row pool is the artifact the v2.0.2 jury actually ran on, and the 79-row adjudication queue plus the philologist handoff are keyed to it. Replacing it would orphan a completed jury run and the human-adjudication evidence for a metric nobody has re-measured. The frozen files therefore stay as they are, with the leak documented and pinned; the clean split lands with the Stream A re-run.
