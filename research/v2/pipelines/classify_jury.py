...
# NOTE: the former hardcoded default project is DELETED. Do not reinstate a
# deleted project — a run must set VERTEX_PROJECT_ID to a live project with the target
# Anthropic model enabled in VERTEX_REGION (Opus is NOT on tripcreator-prod as of
# 2026-07; only Haiku 4.5). Empty default fails fast instead of silently 404ing.
...
def _make_anthropic_vertex(model: str) -> Provider:
    ...
    def invoke(system: str, user: str) -> str:
        ...
        # max_tokens=1024 was the root cause of the retracted v2.0.2 lacuna
        # "Sonnet 0.949 hallucination" result: the lacuna prompt makes the model
        # echo restored_full + rationale + alternates, and the budget could be
        # exhausted (partly on reasoning) before any text block was emitted,
        # yielding resp.content with no text -> "". 4096 gives headroom. We retry
        # once on an empty completion and raise on a persistent empty, so the
        # caller records an api_error rather than a silent "" that downstream code
        # mis-scores as a hallucination.
        for _attempt in range(2):
            ...
            if text:
                return text
        raise RuntimeError(
            f"anthropic_vertex empty completion after retry "
            f"(model={model}, stop_reason={getattr(resp, 'stop_reason', '?')})"
        )
...
# All three bill to the same GCP project (now retired); no separate
