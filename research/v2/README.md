...
| **C — Lacunae gold** | 100–200 editor-restored inscriptions with hallucination metric | **v2.0.3 re-run** on 66 dedup clean-gold tasks (Opus 4.8 0.288 span / Gemini 3.1 Pro 0.258 / Gemini 3.5 Flash 0.258 — all n.s.; hallucination Flash 0.545 vs Pro 0.161). ⚠️ v2.0.2 "Finding C" (Sonnet 0.949) **RETRACTED** — empty completions scored as hallucinations; see CHANGELOG [2.0.3] |
...
The harness is designed to extend to four ancient Italic languages, but only Etruscan is implemented; Oscan/Faliscan/Raetian are empty scaffolding (stub config + README, no codebooks or corpus):
...
The eval harness (bootstrap, classify_metrics, lacuna_metrics) is language-agnostic. Adding Oscan etc. is: draft codebook → fill out config YAML → stage corpus in `gs://openetruscan-rosetta/corpus/` → re-run the same Cloud Build with `--substitutions=_LANGUAGE=osc`. In principle no pipeline changes should be needed, but this path has not yet been exercised for any non-Etruscan language.
...
Models handle the easy majority of rows; humans adjudicate the hard minority. Krippendorff α between human adjudicators must reach ≥ 0.80 before the gold set ships.
