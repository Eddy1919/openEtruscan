...
def main(argv: list[str] | None = None) -> int:
    ...
    for model, mrows in by_model.items():
        clean_all = filter_clean(mrows)
        # Non-answers (empty/unparseable responses) are a coverage failure, not a
        # data point. Excluding them prevents API/parse failures from being scored
        # as hallucinations or zero-accuracy answers (retracted v2.0.2 Sonnet bug).
        clean = answered(clean_all)
        n_no_parse = len(clean_all) - len(clean)
        ...
        summary["per_model"][model] = {
            ...
            "coverage": len(clean) / max(len(clean_all), 1),
        ...
        print(f"## {model}  (n={len(clean)}, no_parse={n_no_parse}, dropped={n_dirty})")
    ...
    for i, m_a in enumerate(models):
        for m_b in models[i + 1 :]:
            a_clean = answered(filter_clean(by_model[m_a]))
            b_clean = answered(filter_clean(by_model[m_b]))
