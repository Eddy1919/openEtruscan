...
def answered(rows: Sequence[dict]) -> list[dict]:
    """Drop non-answers (empty/unparseable model responses; no_parse=True).

    A no_parse row means the model returned nothing scoreable. It is a COVERAGE
    failure, not a hallucination and not a zero-accuracy answer — scoring it as
    either corrupts the metric. This is the fix for the retracted v2.0.2 Sonnet
    result. Report the dropped count separately (n_no_parse / coverage).
    """
    return [r for r in rows if not r.get("no_parse")]
