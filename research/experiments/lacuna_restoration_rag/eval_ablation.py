...
def norm(s): return re.sub(r"[^\wθχφσśπ𐌀-𐌿]", "", (s or "").lower())

def answer_window(row):
    """last2(left) + gold + first2(right), normalized — the local copy signal."""
    m=row["masked"]; w=row["width"]; g=R.GOLD[row["key"]]
    start=m.index("?"); left=norm(m[:start]); right=norm(m[start+w:])
...
def run(mid):
    def one(row):
        ...
        up=R.USER_TEMPLATE.format(width=row["width"], masked_text=row["masked"],
                                  parallels=parallels, id_json=json.dumps(row["id"]))
        p=R.parse(R.call(mid,up,0.0), row["width"], row["masked"])
        return dict(key=row["key"], id=row["id"], model=f"{mid}+ragNOLEAK", masked=row["masked"],
                    width=row["width"], width_bucket=row["width_bucket"], gold_lacuna=R.GOLD[row["key"]], **p)
...
if __name__=="__main__":
    ...
    revealed=sum(1 for row in R.POOL if answer_window(row) in "\n".join(norm(p) for p in R.retrieve(row,k=8)))
    print(f"wrote {len(rows)} | tasks whose top-8 contained an answer-revealing parallel: {revealed}/{len(R.POOL)}")
