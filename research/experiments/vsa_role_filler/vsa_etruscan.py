...
STATUS = set("lautni lautniθa lautn clan sec seχ śeχ puia etera papals nefts "
             "cliniiaras cliniar".split())
THEONYM = set("tin tinia uni menrva turan fufluns aritimi hercle herecele nethuns "
...
def parse_roles(toks):
    ...
    for k, i in enumerate(pr123):
        t = toks[i]
        if t in PRAENOMINA and not seen_praenomen:
            roles.append(("PRAENOMEN", t)); used[i]=True; seen_praenomen=True
        elif re.search(r"(al|ial|s|sa|isa|us|es)$", t) and seen_praenomen:
            roles.append(("PATRONYMIC", t)); used[i]=True     # genitive-marked
            ...
            roles.append(("GENTILICIUM", t)); used[i]=True
    # keep original textual order
    order = {id(x): n for n, x in enumerate(roles)}
    return roles
...
print(f"corpus: {len(corpus)} | multi-token: {len(multi)} | parsed into >=2 roles: {len(ge2roles)}"
...
def bind(a, b): return a * b                       # MAP binding (self-inverse, bipolar)

def encode(roles):
    """content vector I = sum role*filler ; structure vector S = sum role*pos."""
    I = np.zeros(D); S = np.zeros(D)
    for k,(role,fill) in enumerate(roles):
        I += bind(role_vec[role], fill_vec[fill])
        S += bind(role_vec[role], POS[min(k,11)])
    return I, S
...
def roundtrip(role):
    ...
    for p in parsed:
        ...
        tot += 1; ok += int(rec == gt[-1])
    return ok, tot
for role in ["GENTILICIUM","PATRONYMIC","STATUS"]:
    ok,tot = roundtrip(role)
    if tot: print(f"round-trip unbind {role:12}: {ok}/{tot} = {100*ok/tot:.1f}%")
...
Smat = np.stack([p["S"] for p in pool]); Smat /= (np.linalg.norm(Smat,axis=1,keepdims=True)+1e-9)
for p in pool: p["ng"]=ngrams(p["canon"])
roleseq = lambda p: tuple(r for r,_ in p["roles"])
tokset  = lambda p: set(p["toks"])
def jacc(a,b): return len(a&b)/len(a|b) if (a|b) else 0.0

rs = rng.choice(len(pool), size=min(120,len(pool)), replace=False)
vsa_struct=[]; vsa_lex=[]; ng_struct=[]; ng_lex=[]
for qi in rs:
    q = pool[qi]
    # VSA structural top-5
    sims = Smat @ (q["S"]/ (np.linalg.norm(q["S"])+1e-9)); sims[qi]=-9
    top = np.argsort(-sims)[:5]
    vsa_struct += [jacc(set(roleseq(q)), set(roleseq(pool[j]))) for j in top]
    vsa_lex    += [jacc(tokset(q), tokset(pool[j])) for j in top]
    # char-3gram top-5
    ns = np.array([cos_counter(q["ng"], pool[j]["ng"]) if j!=qi else -9 for j in range(len(pool))])
...
print(f"                     role-structure sim   lexical(word) overlap")
...
print("(VSA should give HIGH structure sim at LOW lexical overlap = parallels char-ngram can't see)")

# concrete examples
print("\n=== example: same structure, different words (VSA-found) ===")
shown=0
for qi in rs:
    q=pool[qi]; sims=Smat@(q["S"]/(np.linalg.norm(q["S"])+1e-9)); sims[qi]=-9
    j=int(np.argmax(sims))
    if jacc(tokset(q),tokset(pool[j]))==0 and roleseq(q)==roleseq(pool[j]) and len(q["roles"])>=3:
