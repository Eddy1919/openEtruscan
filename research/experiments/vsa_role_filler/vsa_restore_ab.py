...
PRAENOMINA=set("larθ lart laris lars arnθ arnt arunt aule avle vel velθur venel marce mamarce marces sethre seθre śeθre θefarie aθe aθ tite titi vipe caile cae velus aules θana θania θanχvil ramθa ravnθu larθi larθia velia veilia fasti hasti culni seθra ati".split())
STATUS=set("lautni lautniθa lautn clan clenar sec seχ śeχ puia etera papals nefts".split())
THEONYM=set("tin tinia uni menrva turan fufluns aritimi hercle nethuns śuri catha selvans".split())
VERB=set("turce turuce muluvanice muluvanece zinace lupu ame amce".split())
TOMB=set("śuθi suθi θui".split()); EGO=set("mi mini".split())
GEN=re.compile(r"(al|ial|s|sa|isa|us|es|θur|nal|nas|cva|χva)$"); ABBR=re.compile(r"^[a-zθχśφς]{1,2}$")
SEP=re.compile(r"[:·•|/\\\[\]\(\)<>\{\}·•、,\.\s?]+")
norm=lambda s:unicodedata.normalize("NFC",s or"").strip().lower()
def tok(c): return [t for t in SEP.split(norm(c)) if t and not re.fullmatch(r"[-–—\d]+",t)]
def parse(ts):
    roles=[];used=[False]*len(ts)
    for i,t in enumerate(ts):
        for S,nm in [(EGO,"EGO"),(VERB,"VERB"),(TOMB,"OBJECT"),(STATUS,"STATUS"),(THEONYM,"THEONYM")]:
            if t in S: roles.append((nm,t));used[i]=True;break
    seen=False
    for i,t in enumerate(ts):
        if used[i]:continue
        if t in PRAENOMINA and not seen: roles.append(("PRAENOMEN",t));seen=True
        elif ABBR.match(t) and not seen: roles.append(("PRAENOMEN",t));seen=True
        elif GEN.search(t) and seen: roles.append(("PATRONYMIC",t))
        elif not seen: roles.append(("PRAENOMEN",t));seen=True
        else: roles.append(("GENTILICIUM",t))
    return roles

# --- VSA structure vectors over corpus ---
def atom(): return rng.choice([-1.0,1.0],size=D)
ROLES=["EGO","VERB","OBJECT","STATUS","THEONYM","PRAENOMEN","GENTILICIUM","PATRONYMIC"]
rv={r:atom() for r in ROLES}; POS=[atom() for _ in range(14)]
def struct_vec(roleseq):
    S=np.zeros(D)
    for k,role in enumerate(roleseq): S+=rv[role]*POS[min(k,13)]
    return S
cstruct=np.zeros((len(corpus),D)); ccanon=[]; cnorm=[]
for i,c in enumerate(corpus):
    rs=[r for r,_ in parse(tok(c["canonical"]))]
    cstruct[i]=struct_vec(rs); ccanon.append(c["canonical"]); cnorm.append(re.sub(r"\W","",norm(c["canonical"])))
cstruct/= (np.linalg.norm(cstruct,axis=1,keepdims=True)+1e-9)

# --- char-3gram over corpus ---
def ng(s,n=3):
    s=re.sub(r"[^\w\U00010300-\U0001032f]","",norm(s));s=f"^{s}$"
...
def visible(m,w): s=m.index("?"); return m[:s]+m[s+w:]
def gold_full(row): return row["masked"].replace("?"*row["width"], GOLD[row["key"]],1)

def parallels_char(row,k=8):
    q=ng(visible(row["masked"],row["width"])); gf=re.sub(r"\W","",norm(gold_full(row)))
    sims=[(cos_c(q,cng[i]),i) for i in range(len(corpus))]
    out=[]
    for s,i in sorted(sims,reverse=True):
        if cnorm[i] and (cnorm[i] in gf or gf in cnorm[i]): continue
        out.append(ccanon[i]);  # leakage-excluded
        if len(out)>=k: break
    return out
def parallels_vsa(row,k=8):
    rs=[r for r,_ in parse(tok(visible(row["masked"],row["width"])))]
    if len(rs)<2: return []                     # no structure -> no structural parallels
    qv=struct_vec(rs); qv/=np.linalg.norm(qv)+1e-9
    sims=cstruct@qv; gf=re.sub(r"\W","",norm(gold_full(row)))
    out=[]
...
def gemini(prompt):
    body=json.dumps({"system_instruction":{"parts":[{"text":SYS}]},
        "contents":[{"parts":[{"text":prompt}]}],
        "generationConfig":{"temperature":0,"responseMimeType":"application/json",
            "thinkingConfig":{"thinkingBudget":0},"maxOutputTokens":512}}).encode()
    ...
    for _ in range(3):
        try:
            ...
            return json.loads(re.sub(r"^```json?|```$","",t.strip()).strip()).get("restored_lacuna","")
        except Exception: pass
    return None

def clean(g): g=(g or"").strip(); return bool(g) and not re.search(r"-{2,}",g) and not re.search(r"\d",g)
tasks=[r for r in POOL if clean(GOLD[r["key"]])]
def run(row, which):
    par = parallels_char(row) if which=="char" else parallels_vsa(row)
    if which=="vsa" and not par: return ("nostruct", None)
...
resA=score("char"); resB=score("vsa")
def span(pred,row): return pred is not None and pred==GOLD[row["key"]]
A={r["key"]:(st,pr) for r,(st,pr) in resA}; B={r["key"]:(st,pr) for r,(st,pr) in resB}
...
def acc(keys,M):
    ks=[k for k in keys]; return np.mean([span(M[k][1], next(r for r in tasks if r['key']==k)) for k in ks]) if ks else 0
allk=[r["key"] for r in tasks]
print(f"\nspan-exact, ALL {len(allk)} tasks:")
print(f"  char-3gram RAG : {acc(allk,A):.3f}")
print(f"  VSA-structural : {acc([k for k in allk if B[k][0]=='ok']+[k for k in allk if B[k][0]!='ok'],{**B, **{k:A[k] for k in allk if B[k][0]!='ok'}}):.3f}  (falls back to char where no structure)")
