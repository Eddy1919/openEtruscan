...
rng=np.random.default_rng(3); D=4096
corpus=json.load(open("corpus.json"))
PRAENOMINA=set("""larθ lart laris lars arnθ arnt arunt aule avle aφle vel velθur venel marce mamarce
...
tok=lambda c:[t for t in SEP.split(norm(c)) if t and not re.fullmatch(r"[-–—\d]+",t)]
def parse(ts):
    roles=[];used=[False]*len(ts)
    for i,t in enumerate(ts):
        for S,name in [(EGO,"EGO"),(VERB,"VERB"),(TOMB,"OBJECT"),(STATUS,"STATUS"),(THEONYM,"THEONYM")]:
            if t in S: roles.append((name,t));used[i]=True;break
    seen=False
    for i,t in enumerate(ts):
        if used[i]:continue
...
recs=[]
for r in corpus:
    ts=tok(r["canonical"]); roles=parse(ts)
    d={"id":r["id"],"canon":r["canonical"]}
    for role,f in roles: d.setdefault(role,[]).append(f)
    d["roles"]=roles; d["anc"]=any(t in ANCH for t in ts)
    recs.append(d)
        ...
        for patr in d.get("PATRONYMIC",[]):
            ...
            if clean(s) and s in gens_praen.get(g,{}):
                for parent in gens_praen[g][s]:
                    if parent["id"]!=d["id"] and SHORT(parent) and clean(parent.get("PRAENOMEN",[""])[0]):
...
if big:
    ...
    for m in members[:10]:
        pr="·".join(m.get("PRAENOMEN",["?"])); pa="·".join(m.get("PATRONYMIC",[])) or "—"
...
fv={f:atom() for f in fillers}; fmat=np.stack([fv[f] for f in fillers])
pairs=[]
for child,parent,g,s in links:
    cp=child.get("PRAENOMEN",[None])[0]; pp=s
    if cp in fv and pp in fv: pairs.append((cp,pp))
if len(pairs)>=20:
    rng.shuffle(pairs); k=len(pairs)//2
    T=np.zeros(D)                                    # learn "is-child-of" transform
    for cp,pp in pairs[:k]: T+=fv[cp]*fv[pp]
    T=np.sign(T)
    ok=0
    ...
    print(f"  parent recovered: {ok}/{len(pairs)-k} = {100*ok/(len(pairs)-k):.0f}%  (random ~ {100/len(fillers):.2f}%)")
else:
