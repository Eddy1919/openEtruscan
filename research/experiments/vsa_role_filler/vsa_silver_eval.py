...
PRAENOMINA = set("larθ laris arnθ aule avle vel velθur θefarie marce larce marces "
                 "sethre seθre aθ θana θania ramθa larθi larθia velia fasti hasti "
                 "θanχvil culni aules velus arnt lart".split())
STATUS = set("lautni lautniθa lautn clan sec seχ śeχ puia etera papals nefts cliniiaras cliniar".split())
THEONYM = set("tin tinia uni menrva turan fufluns aritimi hercle herecele nethuns śuri catha selvans veive vetis culsu θesan".split())
...
def parse_roles(toks):
    ...
    for i,t in enumerate(toks):
        ...
        elif re.search(r"(al|ial|s|sa|isa|us|es)$",t) and seen: roles.append(("PATRONYMIC",t))
...
data=[]
for r in corpus:
    toks=tokenize(r["canonical"]); roles=parse_roles(toks)
    anchored = any(t in ANCHORS for t in toks)
    if anchored and len(roles)>=3:
        data.append({"id":r["id"],"canon":r["canonical"],"toks":toks,
                     "roleseq":tuple(rr for rr,_ in roles),"wordset":set(toks)})
n=len(data)
...
print("top templates:", [( "·".join(k), len(v)) for k,v in sorted(by_tmpl.items(),key=lambda x:-len(x[1]))[:6]])
...
rv={r:atom() for r in ROLES}; POS=[atom() for _ in range(12)]
...
for i,d in enumerate(data):
    S=np.zeros(D)
    for k,role in enumerate(d["roleseq"]): S+=rv[role]*POS[min(k,11)]
    VSA[i]=S
...
for m in methods:
    a=np.mean(res[m]['all']); s=np.mean(res[m]['structonly'])
...
shown=0
for qi in queries:
    s=VSA@VSA[qi]; s[qi]=-9; j=int(np.argmax(s))
