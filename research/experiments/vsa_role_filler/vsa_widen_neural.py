...
# --- widened lexicons ---
PRAENOMINA = set("""larθ larθ lart laris lars arnθ arnt arunt aule avle aφle vel velθur venel
...
STATUS = set("""lautni lautniθa lautn clan clenar sec seχ śeχ sech puia etera papals papacs nefts
 prumaθs prumts tusurθir tusurθi ati apa papa ruva nefis etnam cliniiaras cliniar""".split())
THEONYM = set("tin tinia uni menrva turan fufluns aritimi hercle herecele nethuns śuri catha selvans veive vetis culsu θesan aplu apulu śuris".split())
VERB = set("turce turuce muluvanice muluvanece zinace mulu alice cerine tece ame amce svalce lupu lupuce".split())
...
def tokenize(c): return [t for t in SEP.split(norm(c)) if t and not re.fullmatch(r"[-–—\d]+", t)]

def parse_roles(toks):
    roles=[]; used=[False]*len(toks)
    for i,t in enumerate(toks):
        if t in EGO: roles.append(("EGO",t)); used[i]=True
        elif t in VERB: roles.append(("VERB",t)); used[i]=True
        elif t in TOMBWORD: roles.append(("OBJECT",t)); used[i]=True
        elif t in STATUS: roles.append(("STATUS",t)); used[i]=True
        elif t in THEONYM: roles.append(("THEONYM",t)); used[i]=True
    seen=False; anchored=any(t in ANCHORS for t in toks)
    for i,t in enumerate(toks):
        if used[i]: continue
        if t in PRAENOMINA and not seen: roles.append(("PRAENOMEN",t)); seen=True
        elif ABBR.match(t) and not seen: roles.append(("PRAENOMEN",t)); seen=True     # abbreviated praenomen
        elif GEN.search(t) and seen: roles.append(("PATRONYMIC",t))                    # genitive-marked
        elif ABBR.match(t) and seen: roles.append(("PATRONYMIC",t))                    # abbreviated patronymic
...
parsed=[]
for r in corpus:
    toks=tokenize(r["canonical"]); roles,anc=parse_roles(toks)
    parsed.append({"id":r["id"],"canon":r["canonical"],"toks":toks,
                   "roleseq":tuple(rr for rr,_ in roles),"wordset":set(toks),"anc":anc,"nrole":len(roles)})
...
print(f"=== NS1.1-widen coverage ===")
print(f"corpus 5932 | multi-token {len(multi)}")
print(f"anchored >=2 roles: {len(anc2)}  ({100*len(anc2)/5932:.1f}% of corpus)   [was ~ n/a]")
print(f"anchored >=3 roles: {len(anc3)}  ({100*len(anc3)/5932:.1f}% of corpus)   [prev parser: 611 = 10.3%]")

# --- silver eval set: anchored >=3 (richer templates) ---
data=anc3; n=len(data)
by_tmpl=defaultdict(list)
for i,d in enumerate(data): by_tmpl[d["roleseq"]].append(i)
...
def hashvec(items,dim):
    v=np.zeros(dim)
    for it in items:
        h=hash(it); v[h%dim]+= 1.0 if (h//dim)%2 else -1.0
    return v
def cngrams(s):
    s=re.sub(r"[^\w\U00010300-\U0001032f]","",norm(s)); s=f"^{s}$"
    return [s[i:i+3] for i in range(len(s)-2)] if len(s)>=3 else [s]
VSA=np.zeros((n,D)); CH=np.zeros((n,H)); TOK=np.zeros((n,H))
for i,d in enumerate(data):
    S=np.zeros(D)
    for k,role in enumerate(d["roleseq"]): S+=rv[role]*POS[min(k,13)]
    VSA[i]=S; CH[i]=hashvec(cngrams(d["canon"]),H); TOK[i]=hashvec(d["toks"],H)

# --- NEURAL baseline: gemini-embedding-001 (768d) ---
KEY=os.environ.get("GEMINI_API_KEY","")
def embed(text):
    body=json.dumps({"content":{"parts":[{"text":text[:512]}]},"outputDimensionality":768}).encode()
    req=urllib.request.Request(
        f"https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-001:embedContent?key={KEY}",
        data=body, headers={"Content-Type":"application/json"})
    for _ in range(3):
        try:
            d=json.load(urllib.request.urlopen(req,timeout=30)); v=d.get("embedding",{}).get("values")
            if v: return np.array(v,dtype=float)
...
methods={"VSA structure":l2(VSA),"gemini-embed":l2(GEM) if GEM is not None else None,
         "char-3gram":l2(CH),"token-overlap":l2(TOK),"random":l2(rng.standard_normal((n,H)))}
...
queries=[i for v in multi_t.values() for i in v]; rng.shuffle(queries); queries=queries[:600]
res={m:{"all":[],"so":[]} for m in methods}; nsq=0
for qi in queries:
    pos=set(by_tmpl[data[qi]["roleseq"]])-{qi}
    so={j for j in pos if not (data[qi]["wordset"]&data[j]["wordset"])}
    if so: nsq+=1
    for m,Mat in methods.items():
        r=recall_at(Mat,qi,pos);  res[m]["all"].append(r) if r is not None else None
        if so: res[m]["so"].append(recall_at(Mat,qi,so))
