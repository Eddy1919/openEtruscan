...
> **Note.** The 38.0% / 60.6% figures are point estimates on a single
> 500-row eval, reported without confidence intervals; small differences
...
scale. Production restoration now uses the retrieval-augmented (RAG)
restorer (see [`../lacuna_restoration_rag/`](../lacuna_restoration_rag/));
this XLM-R + char-head was a candidate approach, not the current prod
model. Approach A is preserved as a documented negative baseline
alongside ByT5 v4/v5.
