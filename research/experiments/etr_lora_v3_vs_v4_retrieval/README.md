...
Qualitative signals are promising, but re-embedding is not yet justified on measured retrieval quality (precision@k pending). The qualitative gains are concentrated exactly where the cleaning intervened (sibilant unification, mirror-glyph removal, Old Italic regeneration), consistent with the encoder having learned some orthographic-equivalence structure; quantitative retrieval metrics are pending — not yet confirmed.
