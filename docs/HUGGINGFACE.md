...
|---|---|---|
| `classifier_v2/` | `src/openetruscan/ml/classifier.py` (TF-IDF + MultinomialNB) trained on the v2 train pool | TF-IDF + NB: **macro F1 0.313, 95 % CI 0.273 – 0.348** on n=143 candidate-gold rows. Three neural baselines on the same split: CharCNN 0.369 [0.257, 0.432]; MicroTransformer 0.317 [0.202, 0.404]; EmbeddingMLP (frozen MiniLM-multilingual) 0.124 [0.099, 0.149]. ⚠️ All four are **upper bounds**: the shared split is id-disjoint but not text-disjoint (25/400 test rows repeat a train text under another id), and the leak concentrates in the n=143 scored subset. Not retracted; not clean either. See [Deviation §D](../research/v2/PRE_REGISTRATION.md). |
