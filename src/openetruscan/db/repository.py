...
class InscriptionRepository:
    ...
    async def search_radius(
        ...
        stmt = (
            select(Inscription)
            .where(
                text("ST_DWithin(geom::geography, ST_GeogFromText(:center), :radius)").bindparams(
    ...
    async def validate_pleiades_ids(self) -> dict[str, Any]:
        ...
        """
        stmt = select(Inscription.id, Inscription.pleiades_id).where(
    ...
    async def add(self, inscription_dataclass: InscriptionData) -> str:
        ...
        flags_str = (
            ",".join(inscription_dataclass.provenance_flags)
            if inscription_dataclass.provenance_flags
            else ""
        )
    ...
    async def get_concordance_network(self, inscription_id: str) -> Sequence[Inscription]:
        ...
        stmt = select(Inscription).where(
            or_(
                ...
                and_(
                    Inscription.trismegistos_id.is_not(None),
                    Inscription.trismegistos_id.in_(shared_ids),
                ),
    ...
    async def get_names_network(self, inscription_id: str) -> dict[str, Any]:
        ...
        rel_stmt = (
            ...
            .where(
                or_(
                    Relationship.person_id.in_(entity_ids),
                    Relationship.related_person_id.in_(entity_ids),
                )
            )
            .options(
        ...
        nodes[f"ins:{inscription_id}"] = {
            "id": f"ins:{inscription_id}",
            "label": inscription_id,
            "type": "inscription",
        }
        ...
        for rel in relationships:
            ...
            if rel.clan:
                ...
                if rel.person_id:
                    edges.append(
                        {"from": rel.person_id, "to": rel.clan_id, "label": rel.relationship_type}
                    )
    ...
    async def concordance(
        ...
        result = await self.session.execute(stmt)
        return [
            {"inscId": row[0], "pre": row[1], "match": row[2], "post": row[3]}
    ...
    async def search_clan_members(self, gens: str) -> SearchResults:
        ...
        stmt = (
            ...
            .join(
                Relationship,
                or_(
                    Relationship.person_id == Entity.id, Relationship.related_person_id == Entity.id
                ),
            )
            .join(Clan)
            .where(Clan.name.ilike(f"%{gens}%"))
            .limit(500)
        )
    ...
    async def get_stats_summary(self) -> dict[str, Any]:
        ...
        summary["top_sites"] = [
            {"findspot": r[0], "count": r[1]} for r in top_sites_result.fetchall()
        ]
    ...
    def _to_dataclass(self, model: Inscription) -> InscriptionData:
        ...
        return InscriptionData(
            ...
            findspot_uncertainty_m=int(model.findspot_uncertainty_m)
            if model.findspot_uncertainty_m is not None
            else None,
            ...
            original_script_entry=model.original_script_entry,
        )
