...
down_revision: Union[str, Sequence[str], None] = '12beabe2bd09'
...
def upgrade() -> None:
    ...
    op.create_table('relationships',
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.CheckConstraint('(related_person_id IS NOT NULL AND clan_id IS NULL) OR (clan_id IS NOT NULL AND related_person_id IS NULL)', name='check_relationship_target'),
    ...
    op.create_index('ix_relationships_related_person_id', 'relationships', ['related_person_id'], unique=False)
