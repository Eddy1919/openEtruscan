...
class Inscription:
    ...
    def to_dict(self) -> dict:
        """Serialize the inscription to a dictionary for API and export formats."""
        return {
            "id": self.id,
...
class SearchResults:
    ...
    def __len__(self) -> int:
        """Return the number of inscriptions in the collection."""
        return len(self.inscriptions)

    def __iter__(self) -> Iterator[Inscription]:
        """Iterate over the inscriptions in the collection."""
    ...
    def _to_csv(self) -> str:
        """Export the collection to a CSV string with standard header mapping."""
        import io
    ...
    def _to_geojson(self) -> str:
        """Export the search results as a GeoJSON FeatureCollection for map rendering."""
        features = []
        for i in self.inscriptions:
...
CREATE INDEX IF NOT EXISTS idx_emb_text_hnsw
    ON inscriptions USING hnsw ((emb_text::halfvec(3072)) halfvec_cosine_ops);
CREATE INDEX IF NOT EXISTS idx_emb_context_hnsw
    ON inscriptions USING hnsw ((emb_context::halfvec(3072)) halfvec_cosine_ops);
CREATE INDEX IF NOT EXISTS idx_emb_combined_hnsw
    ON inscriptions USING hnsw ((emb_combined::halfvec(3072)) halfvec_cosine_ops);
"""
...
def _extract_names(canonical: str) -> list[str]:
    """
    Extract individual names/entities from a canonical transcription string.
    Uses whitespace and punctuation as delimiters.
    """
    import re as _re

    # Split by common word separators and punctuation marks
...
class Corpus:
    ...
    def __init__(self, dsn: str) -> None:
        """Initialize the PostgreSQL corpus connection using the provided DSN."""
        try:
    ...
    def _prepare_inscription(self, inscription: Inscription, language: str) -> Inscription:
        """
        Normalize and validate an inscription before insertion.
    ...
    def _ensure_db(self) -> None:
        ...
        with self._conn.cursor() as cur:
            ...
            try:
                ...
                cur.execute(
                    "ALTER TABLE inscriptions ADD COLUMN IF NOT EXISTS emb_text vector(3072);"
                )
                cur.execute(
                    "ALTER TABLE inscriptions ADD COLUMN IF NOT EXISTS emb_context vector(3072);"
                )
                cur.execute(
                    "ALTER TABLE inscriptions ADD COLUMN IF NOT EXISTS emb_combined vector(3072);"
                )
        ...
        for start in range(0, len(inscriptions), batch_size):
            ...
            with self._conn.cursor() as cur:
                psycopg2.extras.execute_values(
                    cur, query, rows, template=template, page_size=batch_size
                )
            total += len(chunk)
    ...
    def _build_search_query(
        ...
        """Construct a parameterized PostgreSQL SQL query based on filters."""
        conditions = []
        params = []
    ...
    def mvt_tiles(self, z: int, x: int, y: int) -> bytes:
        ...
        query = """
            ...
            mvtgeom AS (
                ...
                WHERE i.geom IS NOT NULL AND ST_Intersects(i.geom, ST_Transform(bounds.geom, 4326))
            )
    ...
    def get_stats_summary(self) -> dict:
        """Generate an overview of corpus statistics (counts, locations, types)."""
        import psycopg2.extras
    ...
    def get_stats_timeline(self) -> dict:
        """Return chronological distribution of inscriptions for timeline visualizations."""
        import psycopg2.extras
...
def _safe_float(val: str | None) -> float | None:
    """Safe conversion of string/none to float for coordinate and measurement fields."""
...
def _safe_int(val: str | None) -> int | None:
    """Safe conversion of string/none to integer for ID and year fields."""
    if not val or val.strip() == "":
        return None
    try:
