...
def auto_label_image(image_path: str, known_text: str, output_dir: str):
    ...
    if len(sorted_bboxes) < len(clean_text):
        logger.warning(
            f"Found fewer boxes ({len(sorted_bboxes)}) than text chars ({len(clean_text)}). Some letters may be eroded."
        )
    ...
    for i, bbox in enumerate(sorted_bboxes):
        ...
        if char in CHAR_TO_CLASS:
            ...
            yolo_labels.append(
                f"{class_id} {bbox['x_center_norm']:.6f} {bbox['y_center_norm']:.6f} {bbox['w_norm']:.6f} {bbox['h_norm']:.6f}"
            )
