...
### Changed
- **Zenodo dataset deposit v1.1.0 published: version DOI
  10.5281/zenodo.21854263.** Adds `openetruscan_clean_grouped.csv` (the
  6,567 rows plus `dup_group_id` / `dup_group_size`, generated by
  `add_dup_group_id.py`) alongside a byte-identical
  `openetruscan_clean.csv`, and replaces the record description (which now
  documents the duplicate-text leak surface and tells consumers to split on
  groups). Publishing surfaced a latent metadata bug inherited from v1.0.0:
  a `related_identifiers` entry carried the full Larth bibliographic
  citation as its "identifier", which fails scheme validation and broke
  every metadata round-trip; it is now the paper's ACL Anthology URL.
  Manifest, CITATION.cff, codemeta.json, README, the data READMEs, and
  `fetch_data.py` (which gains the grouped file, pinned to the new record;
  the original stays pinned to the v1.0.0 record its reproduction chain
  cites) all updated.
